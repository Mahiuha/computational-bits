...
export const validate_program = (program, variables) => {
  ...
  for (let i = 0; i < program.length; i++) {
    ...
    if (instruction.args.var !== undefined) {
      ...
      if (!found) {
        return {isValid: false, error: `The rover does not have a variable called ${instruction.args.var}`};
      }
    }
    ...
    switch (instruction.block) {
      ...
      case BLOCK_NAMES.REPEAT:
        ...
        if (isNaN(count)) {
          return {isValid: false, error: 'The rover only understands an integer number of times to repeat!'};
        }
      ...
      case BLOCK_NAMES.END_REPEAT:
        top = stack.pop();
        if (top === undefined) {
          return {isValid: false, error: 'The rover can only End Repeat after a matching Repeat instruction.'};
        }
        else if (top.type !== BLOCK_NAMES.REPEAT) {
          return {isValid: false, error: 'The rover tried to End Repeat, but it looks like some other part of the program needs to be resolved first.'};
        }
      ...
      case BLOCK_NAMES.ELSE:
        top = stack.pop();
        if (top === undefined) {
          return {isValid: false, error: 'The rover can only have an Else after a matching If instruction.'};
        }
        else if (top.condition !== true) {
          return {isValid: false, error: 'The rover tried to Else, but it looks like some other part of the program needs to be resolved first.'};
        }
      ...
      case BLOCK_NAMES.END_IF:
        top = stack.pop();
        if (top === undefined) {
          return {isValid: false, error: 'The rover can only End If after a matching If instruction.'};
        }
        else if (top.condition !== true) {
          return {isValid: false, error: 'The rover tried to End If, but it looks like some other part of the program needs to be resolved first.'};
        }
        augmentedProgram[top.line].meta.jumpTo = i + 1;
  ...
  if (top !== undefined) {
    if (top.type === BLOCK_NAMES.REPEAT) {
      return {isValid: false, error: 'The rover found a Repeat instruction, but it needs a matching End Repeat instruction so that it knows when to stop repeating.'};
    }
    else if (top.condition === true) {
      return {isValid: false, error: 'The rover found an If instruction, but it needs a matching End If instruction so that it knows when the condition ends.'};
    }
    else {
      // TODO: Shouldn't ever come up, but make this error more descriptive just in case.
      return {isValid: false, error: 'The rover found a mistake in the program.'};
    }
  }
